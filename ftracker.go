...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calories := ((runningCaloriesMeanSpeedMultiplier * avgSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	avgSpeed := meanSpeed(action, duration) // Вычисление средней скорости.
	mInSec := avgSpeed * kmhInMsec          // Перевод скорости из км/ч в м/с.
	hInM := height / cmInM                  // Перевод роста из сантиметров в метры.
	calories := (walkingCaloriesWeightMultiplier*weight + ((math.Pow(mInSec, power) / hInM) * walkingSpeedHeightMultiplier * weight)) * duration * minInH
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swimmingAvgSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
	calories := (swimmingAvgSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calories
}
