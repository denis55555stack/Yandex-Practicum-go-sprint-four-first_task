...
var (
    ...
    countPool = 10 // сколько раз пользователь переплыл бассейн.
)
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    ...
    calories := ((runningCaloriesMeanSpeedMultiplier * meanSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm 
    * duration * minInH)
    return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    meanSpeed := meanSpeed(action, duration)
    meanSpeedKmhInMsec := meanSpeed * kmhInMsec // Средняя скорость в метрах в секунду
    calories := ((walkingCaloriesWeightMultiplier * weight + (math.Pow(meanSpeedKmhInMsec, 2) / height)
